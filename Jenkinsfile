  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --package-branch fiat Cyrus-serendipity-branch --package-branch tsfc cyrus-serendipity --package-branch ufl cyrus-serendipity --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install pyadjoint --no-package-manager || (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }
